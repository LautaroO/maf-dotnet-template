...
Customize this section when adopting the template in a concrete repository.

Document at least:

- the solution and project structure;
- target .NET SDK and MAF package versions;
- build, format, test, and run commands;
- agent, workflow, tool, middleware, and provider-adapter locations;
- persistence, checkpoint, memory, and hosting choices;
- project-specific security, privacy, and approval constraints;
- intentionally supported provider-specific capabilities.

Do not weaken the provider-neutral architecture rules below. If a project intentionally depends on a provider-specific capability, document the exception, isolate it behind an adapter, and explain its portability impact.
